...
def mh_secondary_remove_and_transform(df, col_lst=['MHLTH_CrudePrev', 'MHLTH_Crude95CI'], trans_col='Geolocation'):
    """
...
def mh_plotly_treemap(df, city_level=False, path_lst=[px.Constant('US'), 'StateAbbr', 'PlaceName'], color='MHLTH_AdjPrev', values='MHLTH_AdjPrev', style='Blues', title='Mental Health Prevalence by State and City', width=1000, height=600):
    ...
    Parameters:
    df: dataframe
    city_level: boolean, if True, the treemap will be plotted at city level
    path_lst: list, the path of the treemap
        to have a constant parent node, use px.Constant()
...
def mh_apply_boundary(df, new_geo_col, boundary, key_col='StateAbbr'):
    """
    Apply boundary to the dataframe and return the mean of the dataframe grouped by the new_geo_col
    """
...
def mh_pop_vs_mh(df, x_col='CitySize', bar_ycol='Population2010', point_ycol='square_MHLTH_AdjPrev', y_title='Number of Cities and Average (squared) MH Prevalence', title='Population vs MH Prev', width=500, height=200):
    """
