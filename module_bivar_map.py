...
def mikhailsirenko_colorscale(percentile = np.linspace(0.25, 1, 4), color_list = ['#e8e8e8','#be64ac','#5ac8c8','#3b4994']):
    """
    ...
    def hex_to_Color(hexcode):
        ...
        rgb = Color(*rgb) # * unpacks the list into arguments, converts RGB values to Color object
    ...
    for i in range(num_grps): # creating top and bottom horizontal color gradient
        ...
        c00_to_c10.append(c00.lerp(c10, 1/(num_grps) * i))
        c01_to_c11.append(c01.lerp(c11, 1/(num_grps) * i))

    for i in range(num_grps): # filling in the grid vertically
        for j in range(num_grps):
            colorlist.append(c00_to_c10[i].lerp(c01_to_c11[i], 1/(num_grps) * j))

    # convert colorlist(rgba) back to RGB values
    colorlist = [[c.r, c.g, c.b] for c in colorlist]

    color_list_4x4 = np.array(colorlist[::-1]).reshape(4,4,3)
    return color_list_4x4

def assign_color_cells(df, env_col, env_color_01, mh_col, mh_color_02, percentile = np.linspace(0.25, 1, 4).tolist()):
    """
...
def matplotlib_map(ax, df,color_col_01, color_col_02, color_list, xlim = [-123,-115], ylim = [32,40],alpha = 1, edge_color = 'black', line_width = 0.35, figsize = (20,10)):
    """
    Create bivariate choropleth map using matplotlib
    """
    ax.set_xlim(xlim[0],xlim[1])
    ax.set_ylim(ylim[0],ylim[1])
    df.plot(ax=ax, legend=False, color = color_list[df[color_col_01], df[color_col_02]],alpha = alpha, edgecolor=edge_color, linewidth=line_width)
...
def bicolor_legend(ax, color_list, legend_position = [0.6,0.7,0.2,0.2], tick_fontsize = 6, label_fontsize = 8, x_label = 'Mental Health', y_label = 'Total Area of Greenness'):
    """
    ...
    default_ticks = np.arange(0, 5, 1)
    adjusted_ticks = default_ticks - 0.5
    ax.set_xticks(adjusted_ticks)
    ax.set_xticklabels([1,0.75,0.5,0.25,0], fontsize=tick_fontsize)
    ax.set_yticks(adjusted_ticks)
    ax.set_yticklabels([1,0.75,0.5,0.25,0],fontsize=tick_fontsize)
