...
#### Below functions are for data manipulation for bivariate choropleth map
...
def df_manipulation_for_bimap(geo_path, merged_path, env_feature, merge_on='UC_Grouping',
percentile = np.linspace(0.33, 1, 3), color_list=['#ffb000', '#dc267f', '#648fff', '#785ef0'],
    env_color_01 = 'c1_env',
    mh_col = 'MH_Score',
    mh_color_02 = 'c2_mh'):
    """
    ...
    Consolidated functions:
        ...
        - assign_color_cells
    """
    geo_df = merge_geo_df(geo_path, merged_path, merge_on)
    focused_df = df_focused_env_feature(geo_df, env_feature)
    normalized_df = normalize_features(focused_df, env_feature)
...
def merge_geo_df(geo_path, merged_path, merge_on='UC_Grouping'):
    geo_us = gpd.read_file(geo_path)
    merged_df = pd.read_csv(merged_path, index_col=0)
    merge_geo_df = merged_df.merge(geo_us, on= merge_on, how='left')
    geo_df = gpd.GeoDataFrame(merge_geo_df, geometry='geometry')
    return geo_df

def df_focused_env_feature(gdf, env_feature):
    focused_df = gdf[['geometry', 'MH_Score', env_feature]]
    return focused_df

def normalize_features(df, env_feature):
    """
    Normalize the features to [0, 1] range
    In order to present the features in the same scale
    """
    indf = df.copy()
    for feature_name in [env_feature, 'MH_Score']:
